...
export class Grammar{
    ...
    static fromCompiled(rules, start) {
        ...
        var rules = rules.map(({name, symbols, postprocess}) => new Rule(name, symbols, postprocess));
    ...
    feed(chunk) {
        ...
        while (true) {
            try {
                ...
                var err = new ParserError(this.reportLexerError(e));
            ...
            if (nextColumn.states.length === 0) {
                ...
                var err = new ParserError(this.reportError(token));
                err.offset = this.current;
                err.token = token;
                throw err;
            }
