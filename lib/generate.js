...
function tabulateString (string, indent, options) {
    ...
    options = options || {};
    const tabulated = lines.map(function addIndent (line, i) {
...
generate.js = generate.javascript = (parser, exportName) => {
...
generate._default = generate.module = generate.esmodule = (parser, exportName) => {
    let output = `// Generated automatically by nearley, version ${parser.version}\n`;
    output += "// http://github.com/Hardmath123/nearley\n";
    output += "function id(x) { return x[0]; }\n";
    output += parser.body.join('\n');
    output += `const Lexer = ${parser.config.lexer};\n`;
    output += `const ParserRules = ${serializeRules(parser.rules, generate.javascript.builtinPostprocessors)};\n`;
    output += `const ParserStart = ${JSON.stringify(parser.start)};\n`;
    output += "export default { Lexer, ParserRules, ParserStart };\n";
    return output;
};

generate.cs = generate.coffee = generate.coffeescript = (parser, exportName) => {
    let output = `# Generated automatically by nearley, version ${parser.version}\n`;
    output += "# http://github.com/Hardmath123/nearley\n";
    output += "do ->\n";
    output += "  id = (d) -> d[0]\n";
...
function* cartesianProduct (a, ...more) {
    ...
    for (const v of a)
        for (const c of cartesianProduct(...more)) // ⚠️
            yield [v, ...c];
}
...
generate.tsd = ({version, body, rules}, exportName) => {
    ...
    output += "const __A = __a as readonly [any, any, any, any, any, any, any, any,...any[]];\n";
    ...
    function* expandRules (name, depth = defaultDepth) {
        for (const rule of map.get(name)) {
            ...
            for (const combination of cartesianProduct(
                ...rule.symbols.map(
                    i => typeof i === 'string'
                        ? expandRules(i, i.includes('$m') ? depth - defaultDepth / macroDepth : depth - 1)
                        : typeof i === 'object' && i?.token
                            ? ['__t']
                            : ['__a']
                )
            )) {
                yield `${rule.postprocess}([${combination.map(normalize).join(', ')}] as const)`;
            }
        }
    }
