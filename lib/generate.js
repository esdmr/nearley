(function(root, factory) {
    ...
    generate.ts = generate.typescript = function (parser, exportName) {
        ...
        output +=  "// @ts-ignore\n";
        output += "function id<T>(d: [T, ...any[]]): T { return d[0]; }\n";
        ...
        output += "};\n";
        output += "\n";
        output += "type NearleySymbol = string | { type: string } | { literal: any } | { test: (token: any) => boolean };\n";
        output += "\n";
    ...
    function* cartesianProduct(...iters) {
        if (iters.length === 0) {
            yield [];
            return;
        }

        const [head, ...tail] = iters;
        const tailProduct = Array.from(cartesianProduct(...tail));

        for (const newHead of head) {
            for (const newTail of tailProduct) {
                yield [newHead, ...newTail];
            }
        }
    }

    generate.tsd = function (parser) {
        var output = "// Generated automatically by nearley, version " + parser.version + "\n";
        output +=  "// @ts-ignore\n";
        output += "declare function id<T>(d: [T, ...any[]]): T;\n";
        output += "declare function __c<P extends (d: any, a: any, b: any) => any>(p: P, d: Parameters<P>[0] extends undefined ? any[] : Parameters<P>[0]): ReturnType<P>;\n";
        output += parser.body.join('\n');
        ...
        for (const rule of parser.rules) {
            if(rule.postprocess?.builtin) {
                rule.postprocess = generate.typescript.builtinPostprocessors[rule.postprocess.builtin];
            }
        ...
        for (const [name, rules] of map) {
            output += `// ${name}\n`;

            for (const rule of rules) {
                if (!rule.postprocess) continue;

                for (const combination of cartesianProduct(...rule.symbols.map(i => typeof i === 'string' ? map.get(i) : typeof i === 'object' && i?.token ? [token] : [undefined]))) {
                    output += '__c((' + rule.postprocess + '),[' + combination.map(i => i === token ? '0 as unknown as Token' : i?.postprocess ? '__c(' + i.postprocess + ', 0 as any)' : '0 as any') + ']);\n';
                }
            }
        }
